...
@runner.runner_class('dify-service-stream-api')
class DifyServiceStreamAPIRunner(runner.RequestRunner):
    ...
    def __init__(self, ap: app.Application, pipeline_config: dict):
        ...
        print('dify-service-stream-api')
        self.ap = ap
        self.pipeline_config = pipeline_config

        valid_app_types = ['chat', 'agent', 'workflow']
        if self.pipeline_config['ai']['dify-service-stream-api']['app-type'] not in valid_app_types:
            raise errors.DifyAPIError(
                f'不支持的 Dify 应用类型: {self.pipeline_config["ai"]["dify-service-stream-api"]["app-type"]}'
            )

        api_key = self.pipeline_config['ai']['dify-service-stream-api']['api-key']

        self.dify_client = client.AsyncDifyServiceClient(
            api_key=api_key,
            base_url=self.pipeline_config['ai']['dify-service-stream-api']['base-url'],
        )
    ...
    def _try_convert_thinking(self, resp_text: str) -> str:
        ...
        if self.pipeline_config['ai']['dify-service-stream-api']['thinking-convert'] == 'original':
            return resp_text

        if self.pipeline_config['ai']['dify-service-stream-api']['thinking-convert'] == 'remove':
            return re.sub(
        ...
        if self.pipeline_config['ai']['dify-service-stream-api']['thinking-convert'] == 'plain':
    ...
    def _is_valid_url(self, url: str) -> bool:
        """粗略校验URL格式（可根据需求扩展）"""
        return url.startswith(('http://',  'https://', '/', './')) or '.' in url.split('/')[-1] 
    
    def _extract_markdown_images(self, text: str) -> tuple[str, list[dict]]:
        """从文本中提取markdown格式的图片

        Args:
            text: 包含可能的markdown格式图片的文本

        Returns:
            tuple[str, list[dict]]: 去除图片后的文本和图片URL列表
        """
        image_pattern = r'!\[([^\]]*)\]?\s*\(?\s*([^)\s]+)\s*\)?'
    ...
    async def _preprocess_user_message(self, query: core_entities.Query) -> tuple[str, list[str]]:
        ...
        if self.pipeline_config['ai']['dify-service-stream-api']['is-query'] is False and plain_text=='':
            plain_text='-'
    ...
    async def _chat_messages(self, query: core_entities.Query) -> typing.AsyncGenerator[llm_entities.Message, None]:
        ...
        # 用于存储去除图片后的纯文本内容
        text_without_images = ''
        # 用于存储所有累积的原始文本，以便于在消息结束时正确处理markdown图片
        full_pending_text = ''
        ...
        async for chunk in self.dify_client.chat_messages(
            ...
            if mode == 'workflow':
                ...
                if chunk['event'] == 'message':
                    ...
                    # 处理当前chunk，提取并去除图片标记
                    current_text_without_images, _ = self._extract_markdown_images(answer_text)
                    text_without_images += current_text_without_images
                    
                    if self.pipeline_config['ai']['dify-service-stream-api']['is-stream']:
                        accumulated_data.append(current_text_without_images)
                        if len(accumulated_data) >= batch_size:
                            accumulated_data = []
                            # 流式输出中输出去掉图片的纯文本
                            yield llm_entities.Message(
                    ...
                    # 消息结束时，处理可能存在的完整markdown图片
                    processed_text, images = self._extract_markdown_images(full_pending_text)
    ...
    async def run(self, query: core_entities.Query) -> typing.AsyncGenerator[llm_entities.Message, None]:
        """运行请求"""
        if self.pipeline_config['ai']['dify-service-stream-api']['app-type'] == 'chat':
            async for msg in self._chat_messages(query):
                yield msg
        elif self.pipeline_config['ai']['dify-service-stream-api']['app-type'] == 'agent':
            async for msg in self._agent_chat_messages(query):
                yield msg
        elif self.pipeline_config['ai']['dify-service-stream-api']['app-type'] == 'workflow':
